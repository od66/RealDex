...
try:
    ...
    def sample_farthest_points(points, K=1000):
        ...
        if K >= N:
            return points, torch.arange(N, device=device).unsqueeze(0).repeat(B, 1)
        
        # Simple random sampling as fallback (not optimal but functional)
        indices = torch.randperm(N, device=device)[:K].unsqueeze(0).repeat(B, 1).to(device)
        indices_expanded = indices.unsqueeze(-1).expand(-1, -1, 3).to(device)
        sampled_points = torch.gather(points, 1, indices_expanded)
